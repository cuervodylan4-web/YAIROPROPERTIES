...
export async function getListings(options = {}) {
  ...
  try {
    ...
    if (sparkListings.length) return mergeSupplementalListings(supplementalMatches, sparkListings);
...
function findSupplementalMatches(options = {}) {
  ...
  return supplementalListings.filter((listing) => {
    const haystack = normalizeSearchText([listing.address, listing.streetAddress, listing.title, listing.city, listing.neighborhood].join(" "));
    if (hasAddressSearch) {
      return address
        .split(" ")
